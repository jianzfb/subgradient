...
from subgradient.network.lcx.lcx_func import *
from subgradient.context import *
# from lcx_func import *
import requests
import json
import logging
...
def launch_shootback_proxy(target,flag=''):
  ...
  # 0.step global context (get global varaibels)
  ctx = get_global_context()

  # 1.step apply public bridge port
  try:
    result = ctx.subgradient_server_api.apply(public_key=ctx.subgradient_chain_api.public_key, cmd=flag)
    if result is None or result['result'] == 'fail':
        logging.error('couldnt apply %s public bridge port' % flag)
        exit(-1)

    logging.info('success to apply %s public bridge port %d' % (flag, result['bridge_port']))
    proxy_slaver = Slaver(split_host('%s:%d' % (ctx.subgradient_server_api.ip, result['bridge_port'])),
                          split_host(target),
                          max_spare_count=1)
    proxy_slaver.serve_forever()
...
if __name__ == '__main__':
  ...
  master_address = '127.0.0.1:11000'
  target_address = '127.0.0.1:7080'
  # launch_shootback_proxy(master_address, target_address)
  proxy_slaver = Slaver(split_host('59.110.164.237:%d' % 10000),
                        split_host('127.0.0.1:%d' % 8000),
                        max_spare_count=1)
  proxy_slaver.serve_forever()
